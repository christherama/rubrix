...
class DataModel {
	 ...
	 */
	public static function find($className, $fields=null, $orderBy=null) {
	 ...
	 */
	public static function exec($sql,$values,$returnObjectType=null) {
		// Connect to DB
		$conn = self::connect();
		try {
			// Execute SQL
		...
		if($returnObjectType != null) {
			...
			foreach($resultSet as $row) {
				$rc = new ReflectionClass($returnObjectType);
				$obj = $rc->newInstance();
				$obj->data = $row;
				$result[] = $obj;
			}
...
//			trigger_error($msg,E_USER_NOTICE);
	...
	/**
	 * Validates POST data to ensure required fields are present and nonempty
